...
export class SalesEnquiryComponent implements OnInit {
  ...
  buttons = [
    ...
    { label: 'Delete', action: () => this.delete(), visible: true, disabled: false },
    { label: 'Cancel', action: () => this.showConfirm('Cancel'), visible: true, disabled: false },
    { label: 'Add', action: () => this.showConfirm('Cancel'), visible: true, disabled: false },
  ...
  TableData = [
    ...
    { field: 'slNo', header: 'Sl. No.', hide: false, mandatory: true, width: '3rem', data: "12345", manual: false, bind: "Alld_SlNo", type: 'number' },
    ...
    { field: 'unit', header: 'Unit', hide: false, mandatory: false, width: '7rem', data: '123456', manual: true, bind: "Alld_Prod_Unit", affects: ['Alld_Gross_Amount', 'Alld_Net_Amount'], align: 'left', type: 'text', PopUp: true, PopUpData: 'ProductUnit', },
    ...
    { field: 'warehouse', header: 'Warehouse', hide: false, mandatory: false, width: '8rem', data: '123456789', manual: true, PopUp: true, PopUpData: 'WarehouseCode', bind: "Alld_WhOne_Name", routeTo: "warehouse", type: 'text' },
  ...
  hideOrDisplayFields = [
    ...
    },
    {
      ...
      HFieldMaster_Code: '',
      HFieldMaster_Name: '',
      HDataField_Name: 'Alld_OneOutPut_Perc',
      HFieldValue_Yn: false,
      HFieldHide_Yn: true,
      HFieldDisable_Yn: true,
      HField_Alert_Code: 'ALRT002',
      ...
      HFieldMaster_Code: '',
      HFieldMaster_Name: '',
      HDataField_Name: 'Alld_OneInPut_Perc',
      HFieldValue_Yn: false,
      HFieldHide_Yn: true,
      HFieldDisable_Yn: true,
      HField_Alert_Code: 'ALRT002',
  ...
  showVesselBooking: boolean = false;

  showQAQC: boolean = false;

  showWHManagement: boolean = false;

  apiUrl = apiUrl.salesEnquiry

  isLoading: boolean = false

  ConfMiddleScreenData: any
  ...
  HCDocu_Midd_SysID: any
  warehouseOne: any;
  AdditionalExpensesCustomerAcc: any;
  mainTax: any;
  additionalTax: any;
  taxAcc: any;
  ...
  ngOnInit(): void {
    ...
    this._salesService.headerData.tblTransDetail = [new TblTransDetailDto()]
    this._salesService.headerData.tblSecTransDetail = [new TblSecTransDetailDto()]
    ...
    this._salesService.headerData.tblTaxDetail = [new tblTaxDetailDto()]

    this._salesService.headerData.tblExp = [new TblExpDto()]
    ...
    this._salesService.headerData.tblVesBook = [new TblVesBookDto()]
    this._salesService.headerData.tblVesDetail = [new TblVesDetailDto()]
    this._salesService.headerData.tblShipCost = [new TblShippCostDto()]
    this._salesService.headerData.tblVesContDtChange = [new TblVesContDtChangeDto()]
    ...
    if (!hasId) {
      // New Entry - Show only Save, Cancel, Add, List
      switch(btn.label) {
        case 'Save':
        case 'Cancel':
        case 'Add':
  ...
  getConfMiddleScreenData() {
    let filter = '?where[HCDocu_MiddValid_SysID]=1002&where[HCDocu_MiddValid_Code]=5002'

    this._masterService.getMasterData(apiUrl.SalesMiddleScreen, filter).then((res) => {
      this.ConfMiddleScreenData = res
      if (this.ConfMiddleScreenData && this.ConfMiddleScreenData.HCDocu_load_BaseDoc == 'Y')
        this.hideLink = true
      this.HCDocu_Midd_SysID = this.ConfMiddleScreenData.HCDocu_Midd_SysID
    })

    this._masterService.getMasterData(apiUrl.MiddleCompany).then((res) => {
    })

    this._masterService.getMasterData(apiUrl.MiddleVoucherType).then((res) => {
      this.voucherType = res
    })

    this._masterService.getMasterData(apiUrl.MiddleSalesAcc).then((res) => {
      this.SalesAcc = res
    })

    this._masterService.getMasterData(apiUrl.MiddleCustomerAcc).then((res) => {
      this.CustomerAcc = res
    })

    this._masterService.getMasterData(apiUrl.MiddleShippedTo).then((res) => {
      this.shippedTo = res
    })

    this._masterService.getMasterData(apiUrl.MiddleBilledTo).then((res) => {
      this.billedTo = res
    })


    this._masterService.getMasterData(apiUrl.MiddleNarration).then((res) => {
      this.narration = res
    })
    this._masterService.getMasterData(apiUrl.MiddleSalesRateAgree).then((res) => {
      this.salesAgreement = res
    })
    this._masterService.getMasterData(apiUrl.MiddleCurrency).then((res) => {
      this.currencyLov = res
    })
    this._masterService.getMasterData(apiUrl.MiddleProductModule).then((res) => {
      this.prodModule = res
    })
    this._masterService.getMasterData(apiUrl.MiddleCopyDoc).then((res) => {
      this.copyDocTemp = res
    })

    this._masterService.getMasterData(apiUrl.MiddleWarehouseOne).then((res) => {
      this.warehouseOne = res
    })
    this._masterService.getMasterData(apiUrl.MiddleAdditionalExpensesCustomerAcc).then((res) => {
      this.AdditionalExpensesCustomerAcc = res
    })
    this._masterService.getMasterData(apiUrl.MiddlePaymentTerms).then((res) => {
      this.paymentTerms = res
    })
    this._masterService.getMasterData(apiUrl.MiddleIncoTerms).then((res) => {
      this.incoTerms = res
    })
    this._masterService.getMasterData(apiUrl.MiddleMainTax).then((res) => {
      this.mainTax = res
    })
    this._masterService.getMasterData(apiUrl.MiddleAdditionalTax).then((res) => {
      this.additionalTax = res
    })
    this._masterService.getMasterData(apiUrl.MiddleTaxAcc).then((res) => {
      this.taxAcc = res
    })

    this._salesService.getMasterData(apiUrl.MiddlecostCenter).then((res) => {
      this.costCenter = res
    })
    this._salesService.getMasterData(apiUrl.MiddleprofitCenter).then((res) => {
      this.profitCenter = res
    })
    this._salesService.getMasterData(apiUrl.MiddlesalesMan).then((res) => {
      this.salesMan = res
    })
    this._salesService.getMasterData(apiUrl.Middledivision).then((res) => {
      this.divisionlov = res
    })
    this._salesService.getMasterData(apiUrl.Middledepartment).then((res) => {
      this.departmentlov = res
    })
    this._salesService.getMasterData(apiUrl.Middlejob).then((res) => {
      this.joblov = res
    })
    this._salesService.getMasterData(apiUrl.MiddleotherCentre).then((res) => {
      this.otherCenter = res
    })
    this._salesService.getMasterData(apiUrl.MiddlesalesOrganization).then((res) => {
      this.salesOrglov = res
    })
    this._salesService.getMasterData(apiUrl.MiddledistributionChannel).then((res) => {
      this.distChannel = res
    })
    this._salesService.getMasterData(apiUrl.MiddlesalesOffice).then((res) => {
      this.salesOff = res
    })
    this._salesService.getMasterData(apiUrl.MiddlesalesGroup).then((res) => {
      this.salesGrp = res
    })
    this._salesService.getMasterData(apiUrl.MiddleplaceOfsupply).then((res) => {
      this.placeOfSupply = res
    })
    this._salesService.getMasterData(apiUrl.Middlejurisdiction).then((res) => {
      this.jurisdiction = res
    })

    this._salesService.getMasterData(apiUrl.MiddleHideOrDisplayTab).then((res) => {
      this.scrollableTabs = res
    })
  ...
  getSalesDataById() {
    ...
    this._salesService.getMasterDatabyId(apiUrl.salesEnquiry, this.id).then((res) => {
      this._salesService.headerData = res;
      this.formatAllDateFieldsInObject(this._salesService.headerData);


      const hd = this._salesService.headerData;

      // Initialize missing or empty fields
      this.initializeIfEmpty(hd, 'tblTransDetail', [new TblTransDetailDto()]);
      this.initializeIfEmpty(hd, 'tblSecTransDetail', [new TblSecTransDetailDto()]);
      this.initializeIfEmpty(hd, 'tblExp', [new TblExpDto()]);
      this.initializeIfEmpty(hd, 'TblPTerm', [new TblPTermDto()]);
      this.initializeIfEmpty(hd, 'TblPoDetail', [new TblPoDetailDto()]);
      this.initializeIfEmpty(hd, 'TblInco', [new TblIncoDto()]);
      this.initializeIfEmpty(hd, 'tblTax', [new TbltaxDto()]);
      this.initializeIfEmpty(hd, 'tblTaxAdd', [new TbltaxAddDto()]);
      this.initializeIfEmpty(hd, 'tblTaxDetail', [new tblTaxDetailDto()]);
      this.initializeIfEmpty(hd, 'tblVesBook', [new TblVesBookDto()]);
      this.initializeIfEmpty(hd, 'tblVesDetail', [new TblVesDetailDto()]);
      this.initializeIfEmpty(hd, 'tblShipCost', [new TblShippCostDto()]);
      this.initializeIfEmpty(hd, 'tblVesContDtChange', [new TblVesContDtChangeDto()]);
      this.initializeIfEmpty(hd, 'tblVesDocAttach', [new TblVesDocAttachDto()]);
      this.initializeIfEmpty(hd, 'tblQaQC', []);
      this.initializeIfEmpty(hd, 'tblInspe', []);
      this.initializeIfEmpty(hd, 'tblQaQcDocAttach', []);
      this.initializeIfEmpty(hd, 'tblWhM', []);
      this.initializeIfEmpty(hd, 'tblWhMDetail', []);
      this.initializeIfEmpty(hd, 'tblWhDocAttach', []);
      this.initializeIfEmpty(hd, 'tblDocAttach', [new TblDocAttachDto()]);
      this.initializeIfEmpty(hd, 'tblLCMaster', new TblLCMasterDto());
      this.initializeIfEmpty(hd, 'tblLCDetails', [new TblLCDetailsDto()]);
      this.initializeIfEmpty(hd, 'tblLandedCost', [new HTransLCostDto()]);
      this.initializeIfEmpty(hd, 'tblLandedExp', [new TblLandedExpDto()]);

      if (this._salesService.headerData.tblDocAttach?.length) {
        this._salesService.headerData.tblDocAttach.forEach(obj => obj.HDoc_GridNo = 2000);
      }

      if (this._salesService.headerData.tblVesDocAttach?.length) {
        this._salesService.headerData.tblVesDocAttach.forEach(obj => obj.HDoc_GridNo = 3000);
      }

      if (this._salesService.headerData.tblWhDocAttach?.length) {
        this._salesService.headerData.tblWhDocAttach.forEach(obj => obj.HDoc_GridNo = 4000);
      }

      if (this._salesService.headerData.tblQaQcDocAttach?.length) {
        this._salesService.headerData.tblQaQcDocAttach.forEach(obj => obj.HDoc_GridNo = 5000);
      }
      ...
      .catch((error) => {
        ...
        this._messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to fetch sales data. Please try again later.',
        });
      });
  }


  initializeIfEmpty(obj: any, key: string, defaultValue: any) {
    if (!obj[key] || (Array.isArray(obj[key]) && obj[key].length === 0)) {
      obj[key] = defaultValue;
    }
  }

  formatAllDateFieldsInObject(obj: any, dateFormat = 'yyyy-MM-dd', locale = 'en-IN'): void {
    if (!obj) return;
    ...
    Object.keys(obj).forEach(key => {
        ...
        value.forEach((item: any) => {
          Object.keys(item).forEach(field => {
            ...
            if (field.toLowerCase().includes('date') && val) {
              try {
                item[field] = formatDate(val, dateFormat, locale);
              } catch (e) {
      ...
      // If it's a single object (e.g., tblLCMaster)
      else if (typeof value === 'object' && value !== null) {
        Object.keys(value).forEach(field => {
          const val = value[field];
          if (field.toLowerCase().includes('date') && val) {
            try {
              value[field] = formatDate(val, dateFormat, locale);
            } catch (e) {
              // Ignore
            }
          }
  ...
  ShowPopUp(Type, i?) {
    ...
    switch (Type) {
      ...
      case 'VocherType':
        ...
        break;
      case 'SalesAccount':
        this.popUpService.popUpData = this.SalesAcc;
        break;
      case 'CustomerCode':
        this.popUpService.popUpData = this.CustomerAcc;
        break;
      case 'ShippedTo':
      ...
      case 'ProductCode':
        ...
        break;
      case 'ProductUnit':
        this.popUpService.popUpData = this.productUnit;
        break;
      case 'WarehouseCode':
        this.popUpService.popUpData = this.warehouseOne;
        break;

      case 'ExpAccountCode':
        this.popUpService.popUpData = this.AdditionalExpensesCustomerAcc;
        break;
      ...
      case 'TaxCode':
        this.popUpService.popUpData = this.mainTax;
        break;
      case 'TaxACode':
        this.popUpService.popUpData = this.additionalTax;
        break;
      case 'TaxAddAccountCode':
        this.popUpService.popUpData = this.taxAcc;
        break;
  ...
  selectedItem(event: any) {
    ...
    switch (Type) {
      case 'ShippedTo':
        this._salesService.headerData.tblTransHead.Allh_Shipto_SysID = event.HShippedto_SysID
        this._salesService.headerData.tblTransHead.Allh_Shipto_Code = event.HShippedto_Code
        this._salesService.headerData.tblTransHead.Allh_Shipto_Name = event.HShippedto_Name
        break;
      case 'BilledTo':
        this._salesService.headerData.tblTransHead.Allh_Billto_SysID = event.HBilledto_SysID
        this._salesService.headerData.tblTransHead.Allh_Billto_Code = event.HBilledto_Code
        this._salesService.headerData.tblTransHead.Allh_Billto_Name = event.HBilledto_Name
        break;
      case 'NarrationCode':
        this._salesService.headerData.tblTransHead.Allh_Narra_SysID = event.HNarraMast_SysID
        this._salesService.headerData.tblTransHead.Allh_Narra_Code = event.HNarraMast_Code
        this._salesService.headerData.tblTransHead.Allh_Narra_Name = event.HNarraMast_Name
        break;
      case 'SalesAccount':
        this._salesService.headerData.tblTransHead.Allh_AccSecond_SysID = event.HAccOne_SysID
        ...
        break;
      case 'CustomerCode':
        this._salesService.headerData.tblTransHead.Allh_CusVen_SysID = event.HAccOne_SysID
        ...
        break;
      case 'VocherType':
        this._salesService.headerData.tblTransHead.Allh_VouType_SysID = event.HVouType_SysID
        this._salesService.headerData.tblTransHead.Allh_Vou_Type = event.HVouType_Code
        this._salesService.headerData.tblTransHead.Allh_VouType_Name = event.HVouType_Name

        this._salesService.headerData.tblTransDetail.forEach(item => {
          item.Alld_VouType_SysID = event.HVouType_SysID;
          item.Alld_Vou_Type = event.HVouType_Code;
          item.Alld_VouType_Name = event.HVouType_Name;
        });

        this._masterService.getMasterData(apiUrl.MiddleVoucherNo, `?where[HVouType_Name]=${event.HVouType_Name}`).then((data) => {
          this._salesService.headerData.tblTransHead.Allh_Vou_Number = data[0].NextVoucherNumber
        });

        break;
      case 'SalesAgreementCode':
        this._salesService.headerData.tblTransHead.Allh_SaleAgree_SysID = event.HSalAgree_SysID
        this._salesService.headerData.tblTransHead.Allh_SaleAgree_Code = event.HSalAgree_Code
        this._salesService.headerData.tblTransHead.Allh_SaleAgree_Name = event.HSalAgree_Name
        break;
      case 'ProductModuleCode':
        this._salesService.headerData.tblTransHead.Allh_AllModule_SysID = event.HProdModule_SysID
        this._salesService.headerData.tblTransHead.Allh_AllModule_Code = event.HProdModule_Code
        this._salesService.headerData.tblTransHead.Allh_AllModule_Name = event.HProdModule_Name

        this._masterService.getMasterData(apiUrl.MiddleProduct, `?where[HProdModule_Code]=${event.HProdModule_Code}`).then((res) => {
          this.product = res
        })
        break;
      case 'CurrencyCode':
        this._salesService.headerData.tblTransHead.Allh_Curr_SysID = event.HCurrFirst_SysID
        this._salesService.headerData.tblTransHead.Allh_Curr_Code = event.HCurrFirst_Code
        this._salesService.headerData.tblTransHead.Allh_Curr_Name = event.HCurrFirst_Name
        this._salesService.headerData.tblTransHead.Allh_Curr_Rate = event.HCurrFirst_Rate
        this.conversionRate = this._salesService.headerData.tblTransHead.Allh_Curr_Rate
        break;

      case 'CopyDocFromTemplateCode':
        this._salesService.headerData.tblTransHead.Allh_DocTemp_SysID = event.HCDTemp_SysID
        this._salesService.headerData.tblTransHead.Allh_DocTemp_Code = event.HCDTemp_Code
        this._salesService.headerData.tblTransHead.Allh_DocTemp_Name = event.HCDTemp_Name
        break;

      case 'ProductCode':
        this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_Prod_SysID = event.HProd_SysID
        ...
        this._masterService.getMasterData(apiUrl.MiddleProductUnit, `?where[HProd_Code]=${event.HProd_Code}`).then((data) => {
          const combinedArray = [
            ...data.TblDProdUnit.map(item => ({
              HFirstUnit_Base_Unit: item.DProd_Base_Unit,
              HFirstUnit_Unit_Frac: item.DProd_Unit_Frac
            })),
            ...data.TblHProdUnit.map(item => ({
              HFirstUnit_Base_Unit: item.HProd_Base_Unit,
              HFirstUnit_Unit_Frac: item.HProd_Unit_Frac
            }))
          ];
          this.productUnit = combinedArray

          if (this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_Prod_Unit == null || this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_Prod_Unit == '' || this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_Prod_Unit == undefined) {
            this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_Prod_Unit = this.productUnit[0].HFirstUnit_Base_Unit

          }
        })
        break;
      case 'ProductUnit':
        this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_Prod_Unit = event.HFirstUnit_Base_Unit
        break;
      case 'WarehouseCode':
        this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_WhOne_SysID = event.HWh_SysID
        this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_WhOne_Code = event.HWh_Code
        this._salesService.headerData.tblTransDetail[this.tableIndex].Alld_WhOne_Name = event.HWh_Name
        break;
      case 'ExpAccountCode':
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Code_SysID = event.HAccOne_SysID
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Code = event.HAccOne_Code
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Name = event.HAccOne_Name
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Narra_SysID = event.HNarraMast_SysID
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Narra_Code = event.HNarraMast_Code
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Narra_Name = event.HNarraMast_Name
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Curr_SysID = event.HCurrFirst_SysID
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Curr_Code = event.HCurrFirst_Code
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Curr_Name = event.HCurrFirst_Name
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Curr_Rate = event.HCurrFirst_Rate
        break;
      case 'ExpNarrationCode':
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Narra_SysID = event.HNarraMast_SysID
        this._salesService.headerData.tblExp[this.tableIndex].ExpS_Narra_Code = event.HNarraMast_Code
      ...
      case 'PaymentTerms':
        this._salesService.headerData.tblTransHead.Allh_PayTerm_SysID = event.HPayTerm_SysID
        this._salesService.headerData.tblTransHead.Allh_PayTerm_Code = event.HPayTerm_Code
        this._salesService.headerData.tblTransHead.Allh_PayTerm_Name = event.HPayTerm_Name
        this._salesService.headerData.tblTransHead.Allh_PayTerm_Days = event.HPayTerm_Days

        this._salesService.headerData.TblPTerm[0].HPay_Term_SysID = event.HPayTerm_SysID
        this._salesService.headerData.TblPTerm[0].HPay_Term_Code = event.HPayTerm_Code
        this._salesService.headerData.TblPTerm[0].HPay_Term_Name = event.HPayTerm_Name
        break;
      case 'IncoTerms':
        this._salesService.headerData.tblTransHead.Allh_IncoTerm_SysID = event.HInco_SysID
        this._salesService.headerData.tblTransHead.Allh_IncoTerm_Code = event.HInco_Code
        this._salesService.headerData.tblTransHead.Allh_IncoTerm_Name = event.HInco_Name

        this._salesService.headerData.TblInco[0].HInco_Term_SysID = event.HInco_SysID
        this._salesService.headerData.TblInco[0].HInco_Term_Code = event.HInco_Code
        this._salesService.headerData.TblInco[0].HInco_Term_Name = event.HInco_Name
        break;

      case 'TaxCode':
        this._salesService.headerData.tblTax[this.tableIndex].HTax_Code_SysID = event.HFirstRateTax_SysID
        this._salesService.headerData.tblTax[this.tableIndex].HTax_Code = event.HFirstRateTax_Code
        this._salesService.headerData.tblTax[this.tableIndex].HTax_Name = event.HFirstRateTax_Name
        this._salesService.headerData.tblTax[this.tableIndex].HTax_InPut_Perc = event.HFirstTax_InputPerc
        this._salesService.headerData.tblTax[this.tableIndex].HTax_OutPut_Perc = event.HFirstTax_OutputPerc
        this.updateTaxFields()
        break;
      case 'TaxACode':
        this._salesService.headerData.tblTaxAdd[this.tableIndex].HTaxA_Code = event.HFirstRateTax_SysID
        this._salesService.headerData.tblTaxAdd[this.tableIndex].HTaxA_Code = event.HFirstRateTax_Code
        this._salesService.headerData.tblTaxAdd[this.tableIndex].HTaxA_Name = event.HFirstRateTax_Name
        this._salesService.headerData.tblTaxAdd[this.tableIndex].HTaxA_InPut_Perc = event.HFirstTax_InputPerc
        this._salesService.headerData.tblTaxAdd[this.tableIndex].HTaxA_OutPut_Perc = event.HFirstTax_OutputPerc
        break;
      case 'TaxAddAccountCode':
        this._salesService.headerData.tblTaxDetail[this.tableIndex].TaxJS_Account_SysID = event.HAccOne_SysID
        this._salesService.headerData.tblTaxDetail[this.tableIndex].TaxJS_Account_Code = event.HAccOne_Code
        this._salesService.headerData.tblTaxDetail[this.tableIndex].TaxJS_Account_Name = event.HAccOne_Name
        break;
      case 'TaxNarrationCode':
        this._salesService.headerData.tblTaxDetail[this.tableIndex].TaxJS_Narra_SysID = event.HNarraMast_SysID
        this._salesService.headerData.tblTaxDetail[this.tableIndex].TaxJS_Narra_Code = event.HNarraMast_Code
        this._salesService.headerData.tblTaxDetail[this.tableIndex].TaxJS_Narra_Name = event.HNarraMast_Name
        break;
      ...
      case 'CostCenterCode':
        this._salesService.headerData.tblTransHead.Allh_CostCent_SysID = event.HCostCent_SysID
        this._salesService.headerData.tblTransHead.Allh_CostCent_Code = event.HCostCent_Code
        this._salesService.headerData.tblTransHead.Allh_CostCent_Name = event.HCostCent_Name
        break;
      case 'ProfitCenterCode':
        this._salesService.headerData.tblTransHead.Allh_ProCent_SysID = event.HProfCent_SysID
        this._salesService.headerData.tblTransHead.Allh_ProCent_Code = event.HProfCent_Code
        this._salesService.headerData.tblTransHead.Allh_ProCent_Name = event.HProfCent_Name
        break;
      case 'SalesmanCode':
        this._salesService.headerData.tblTransHead.Allh_Sman_SysID = event.HSman_SysID
        this._salesService.headerData.tblTransHead.Allh_Sman_Code = event.HSman_Code
        this._salesService.headerData.tblTransHead.Allh_Sman_Name = event.HSman_Name
        break;
      case 'DivisionCode':
        this._salesService.headerData.tblTransHead.Allh_Div_SysID = event.HDivision_SysID
        this._salesService.headerData.tblTransHead.Allh_Div_Code = event.HDivision_Code
        this._salesService.headerData.tblTransHead.Allh_Div_Name = event.HDivision_Name
        break;
      case 'DepartmentCode':
        this._salesService.headerData.tblTransHead.Allh_Dept_SysID = event.HDept_SysID
        this._salesService.headerData.tblTransHead.Allh_Dept_Code = event.HDept_Code
        this._salesService.headerData.tblTransHead.Allh_Dept_Name = event.HDept_Name
        break;
      case 'JobCode':
        this._salesService.headerData.tblTransHead.Allh_Job_SysID = event.HJob_SysID
        this._salesService.headerData.tblTransHead.Allh_Job_Code = event.HJob_Code
        this._salesService.headerData.tblTransHead.Allh_Job_Name = event.HJob_Name
        break;
      case 'OtherCentreCode':
        this._salesService.headerData.tblTransHead.Allh_OtherCent_SysID = event.HOthCentre_SysID
        this._salesService.headerData.tblTransHead.Allh_OtherCent_Code = event.HOthCentre_Code
        this._salesService.headerData.tblTransHead.Allh_OtherCent_Name = event.HOthCentre_Name
        break;
      case 'SalesOrganizationCode':
        this._salesService.headerData.tblTransHead.Allh_SaleOrg_SysID = event.HSalesOrg_SysID
        this._salesService.headerData.tblTransHead.Allh_SaleOrg_Code = event.HSalesOrg_Code
        this._salesService.headerData.tblTransHead.Allh_SaleOrg_Name = event.HSalesOrg_Name
        break;
      case 'DistributionChannelCode':
        this._salesService.headerData.tblTransHead.Allh_DiscChanel_SysID = event.HDistChanel_SysID
        this._salesService.headerData.tblTransHead.Allh_DiscChanel_Code = event.HDistChanel_Code
        this._salesService.headerData.tblTransHead.Allh_DiscChanel_Name = event.HDistChanel_Name
        break;
      case 'SalesOfficeCode':
        this._salesService.headerData.tblTransHead.Allh_SalesOffice_SysID = event.HSoffice_SysID
        this._salesService.headerData.tblTransHead.Allh_SalesOffice_Code = event.HSoffice_Code
        this._salesService.headerData.tblTransHead.Allh_SalesOffice_Name = event.HSoffice_Name
        break;
      case 'SalesGroupCode':
        this._salesService.headerData.tblTransHead.Allh_SalesGroup_SysID = event.HSalGroup_SysID
        this._salesService.headerData.tblTransHead.Allh_SalesGroup_Code = event.HSalGroup_Code
        this._salesService.headerData.tblTransHead.Allh_SalesGroup_Name = event.HSalGroup_Name
        break;
      case 'PlaceOfSupplyCode':
        this._salesService.headerData.tblTransHead.Allh_PlaceSuppl_SysId = event.HPlaceSupply_SysID
        this._salesService.headerData.tblTransHead.Allh_PlaceSuppl_Code = event.HPlaceSupply_Code
        this._salesService.headerData.tblTransHead.Allh_PlaceSuppl_Name = event.HPlaceSupply_Name
        break;
      case 'JurisdictionCode':
        this._salesService.headerData.tblTransHead.Allh_Jurisd_SysID = event.HJurisd_SysID
        this._salesService.headerData.tblTransHead.Allh_Jurisd_Code = event.HJurisd_Code
        this._salesService.headerData.tblTransHead.Allh_Jurisd_Name = event.HJurisd_Name
        break;
      ...
      case 'LandedExpAccountCode':
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Code = event.accountCode
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Name = event.accountName
        break;
      case 'LandedExpNarrationCode':
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Narra_Code = event.narrationMasterCode
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Narra_Name = event.narrationMasterName
        break;
      case 'LandedExpDefaultCurrencyCode':
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Curr_Code = event.HCurr_Code
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Curr_Name = event.HCurr_Name
        this._salesService.headerData.tblLandedExp[this.tableIndex].CSExp_Curr_Rate = event.HCurr_Rate
        break;
  ...
  Save() {
    ...
    // this._salesService.headerData.tblTransDetail = [];
    // this._salesService.headerData.tblSecTransDetail = [];

    // this._salesService.headerData.tblExp = [];
    // this._salesService.headerData.TblPTerm = [];
    // this._salesService.headerData.TblPoDetail = [];
    // this._salesService.headerData.TblInco = [];

    // this._salesService.headerData.tblTax = [];
    // this._salesService.headerData.tblTaxDetail = [];
    // this._salesService.headerData.tblTaxAdd = [];
    ...
    // this._salesService.headerData.tblVesBook = [];
    ...
    // this._salesService.headerData.tblFieldMaster = [];
    // this._salesService.headerData.tblHTabHD = [];

    // this._salesService.headerData.tblLandedExp = [];
    // this._salesService.headerData.tblLandedCost = [];
    ...
    let body = {
      "tblTransHead": this._salesService.headerData.tblTransHead,
      "tblSecTransHead": this._salesService.headerData.tblSecTransHead,
      "tblTransDetail": this._salesService.headerData.tblTransDetail,
      "tblSecTransDetail": this._salesService.headerData.tblSecTransDetail,
      "tblExp": this._salesService.headerData.tblExp,
      "TblPTerm": this._salesService.headerData.TblPTerm,
      "TblPoDetail": this._salesService.headerData.TblPoDetail,
      "TblInco": this._salesService.headerData.TblInco,
      "tblTax": this._salesService.headerData.tblTax,
      "tblTaxAdd": this._salesService.headerData.tblTaxAdd,
      "tblTaxDetail": this._salesService.headerData.tblTaxDetail,

      "tblVesBook": this._salesService.headerData.tblVesBook,
      "tblVesDetail": this._salesService.headerData.tblVesDetail,
      "tblShipCost": this._salesService.headerData.tblShipCost,
      "tblVesContDtChange": this._salesService.headerData.tblVesContDtChange,
      "tblVesDocAttach": this._salesService.headerData.tblVesDocAttach,


      "tblLCMaster": this._salesService.headerData.tblLCMaster,
      "tblLCDetails": this._salesService.headerData.tblLCDetails,

      "tblDocAttach": this._salesService.headerData.tblDocAttach,

      "tblLandedCost": this._salesService.headerData.tblLandedCost,
      "tblLandedExp": this._salesService.headerData.tblLandedExp,


      "tblWhM": this._salesService.headerData.tblWhM,
      "tblWhMDetail": this._salesService.headerData.tblWhMDetail,
      "tblWhDocAttach": this._salesService.headerData.tblWhDocAttach,

      "tblQaQC": this._salesService.headerData.tblQaQC,
      "tblInspe": this._salesService.headerData.tblInspe,
      "tblQaQcDocAttach": this._salesService.headerData.tblQaQcDocAttach,


    }

    this._masterService.saveMasterData(apiUrl.salesEnquiry, body).then((res) => {
      ...
      if (res.success == false) {
        ...
        if (this.id) {
          this.router.navigate(['sales/sales-enquiry'])
        }
  ...
  delete() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this._masterService.deleteData(apiUrl.salesEnquiry, this._salesService.headerData.tblTransHead.Allh_SysID).then((res) => {
  ...
  resetHeaderData() {
    const hd = new TblTransHeadDto();

    // initialize all fields required by your tables
    hd.tblTransHead = new TblFirstTransHeadDto()
    hd.tblSecTransHead = new TblSecTransHeadDto()
    hd.tblTransDetail = [new TblTransDetailDto()];
    hd.tblSecTransDetail = [new TblSecTransDetailDto()];
    hd.tblExp = [new TblExpDto()];
    hd.TblPTerm = [new TblPTermDto()];
    hd.TblPoDetail = [new TblPoDetailDto()];
    hd.TblInco = [new TblIncoDto()];
    hd.tblTax = [new TbltaxDto()];
    hd.tblTaxAdd = [new TbltaxAddDto()];
    hd.tblTaxDetail = [new tblTaxDetailDto()];
    hd.tblVesBook = [new TblVesBookDto()];
    hd.tblVesDetail = [new TblVesDetailDto()];
    hd.tblShipCost = [new TblShippCostDto()];
    hd.tblVesContDtChange = [new TblVesContDtChangeDto()];
    hd.tblQaQC = [new TblQAQCDto()];
    hd.tblInspe = [new TblInspeDto()];
    hd.tblWhM = [new TblWhMDto()];
    hd.tblWhMDetail = [new TblWhMDetailDto()];
    hd.tblDocAttach = [new TblDocAttachDto()];
    hd.tblVesDocAttach = [new TblVesDocAttachDto()];
    hd.tblQaQcDocAttach = [new TblDocAttachDto()];
    hd.tblWhDocAttach = [new TblDocAttachDto()];
    hd.tblLCMaster = new TblLCMasterDto();
    hd.tblLCDetails = [new TblLCDetailsDto()];
    hd.tblLandedExp = [new TblLandedExpDto()];
    hd.tblLandedCost = [new HTransLCostDto()];

    this._salesService.headerData = hd;
    this.tblJournal = [new tblJournalDto()];
  }
  ...
  routeTo(screen?) {
    ...
    if (screen == 'VesselBooking') {
      this.showVesselBooking = true
      this.showQAQC = false
      this.showWHManagement = false
    }
    else if (screen == 'QA/QC') {
      this.showQAQC = true
      this.showWHManagement = false
      this.showVesselBooking = false
    }
    else if (screen == 'WHManagement') {
      this.showWHManagement = true
      this.showVesselBooking = false
      this.showQAQC = false
    }
  ...
  confirmConf() {
    ...
    if (this.headerConf.button === 'Cancel') {
      this.resetHeaderData()
      if (this.id) {
        this.router.navigate(['sales/sales-enquiry'])
      }
    }
    if (this.headerConf.button === 'Delete') {
      // this.showSaveMsg = true
      this.delete()
    }
  ...
  displaySelectedRows() {
    ...
    this._salesService.headerData.tblTransDetail
      ...
      .forEach(item => {
        const tblWhM = new TblWhMDto();
        tblWhM.Whs_SysID = item.Alld_SysID
        tblWhM.WhsS_SysID = 0;
        tblWhM.Whs_Prod_SysID = item.Alld_Prod_SysID;
        tblWhM.Whs_Prod_Code = item.Alld_Prod_Code;
        tblWhM.Whs_Prod_Name = item.Alld_ProdShort_Name;
        tblWhM.Whs_Prod_LName = item.Alld_ProdLong_Name;
        tblWhM.Whs_Unit = item.Alld_Prod_Unit;
        tblWhM.Whs_Qty = item.Alld_Prod_Qty;

        this._salesService.headerData.tblWhM.push(tblWhM);
      });

    this._salesService.headerData.tblTransDetail
      .filter(item => item.selected)
      .forEach(item => {
        const tblQaQC = new TblQAQCDto();
        tblQaQC.QAs_SysID = item.Alld_SysID
        tblQaQC.QAsS_SysID = 0;
        tblQaQC.QAs_Prod_SysID = item.Alld_Prod_SysID;
        tblQaQC.QAs_Prod_Code = item.Alld_Prod_Code;
        tblQaQC.QAs_Prod_Name = item.Alld_ProdShort_Name;
        tblQaQC.QAs_Prod_LName = item.Alld_ProdLong_Name;
        tblQaQC.QAs_Unit = item.Alld_Prod_Unit;
        tblQaQC.QAs_Qty = item.Alld_Prod_Qty;

        this._salesService.headerData.tblQaQC.push(tblQaQC);
      });
  }
  ...
  AddRows() {  // for adding dummy row data when initail page load or when clicked 
    ...
    // if (this._salesService.headerData.tblFieldMaster.length < 4) {
    //   const number = 4 - this._salesService.headerData.tblFieldMaster.length
    //   for (let index = 0; index < number; index++) {
    //     this._salesService.headerData.tblFieldMaster.push(new TblFieldMasterDto());
    //   }
    // }

    // if (this._salesService.headerData.tblHTabHD.length < 4) {
    //   const number = 4 - this._salesService.headerData.tblHTabHD.length
    //   for (let index = 0; index < number; index++) {
    //     this._salesService.headerData.tblHTabHD.push(new TblHTabHDDto());
    //   }
    // }
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        if (table === 'tblTransDetail') {
          ...
          this._masterService.deleteData(apiUrl.salesEnquiry, `first-detai?where[Alld_SysID]=${rowData.Alld_SysID}&where[AlldS_SysID]=${rowData.AlldS_SysID}`).then((res) => {
        ...
        if (table == 'tblExp') {
          this._masterService.deleteData(apiUrl.salesEnquiry, `expense-detail?where[ExpS_SysID]=${rowData.ExpS_SysID}&where[ExpSS_SysID]=${rowData.ExpSS_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              this.isLoading = false
              this._messageService.add({ severity: 'success', summary: 'Row Deleted', detail: res.message });
              this._salesService.headerData.tblExp.splice(index, 1);
              if (this._salesService.headerData.tblExp.length === 0) {
                this.addRow('tblExp', -1);
              }
            }
          }, err => {
            if (err.error.statusCode == 409) {
        ...
        if (table == 'tblTax') {
          this._masterService.deleteData(apiUrl.salesEnquiry, `tax-detail?where[HTax_SysID]=${rowData.HTax_SysID}&where[HTaxS_SysID]=${rowData.HTaxS_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              this.isLoading = false
              this._messageService.add({ severity: 'success', summary: 'Row Deleted', detail: res.message });
              this._salesService.headerData.tblTax.splice(index, 1);
              if (this._salesService.headerData.tblTax.length === 0) {
                this.addRow('tblTax', -1);
              }
            }
          }, err => {
            if (err.error.statusCode == 409) {
        ...
        if (table == 'tblTaxAdd') {
          ...
          this._masterService.deleteData(apiUrl.salesEnquiry, `addititonal-tax-detail?where[HTaxA_SysID]=${rowData.HTaxA_SysID}&where[HTaxAS_SysID]=${rowData.HTaxAS_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              this.isLoading = false
              this._messageService.add({ severity: 'success', summary: 'Row Deleted', detail: res.message });
              this._salesService.headerData.tblTaxAdd.splice(index, 1);
              if (this._salesService.headerData.tblTaxAdd.length === 0) {
                this.addRow('tblTaxAdd', -1);
              }
            }
          }, err => {
            this.isLoading = false
        ...
        if (table == 'tblTaxDetail') {
          this._masterService.deleteData(apiUrl.salesEnquiry, `tax-journal-detail?where[TaxJS_Jour_SysID]=${rowData.TaxJS_Jour_SysID}&where[TaxJSS_Jour_SysID]=${rowData.TaxJSS_Jour_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              this.isLoading = false
              this._messageService.add({ severity: 'success', summary: 'Row Deleted', detail: res.message });
              this._salesService.headerData.tblTaxDetail.splice(index, 1);
              if (this._salesService.headerData.tblTaxDetail.length === 0) {
                this.addRow('tblTaxDetail', -1);
              }
            }
          }, err => {
            if (err.error.statusCode == 409) {
        ...
        if (table == 'tblLCDetails') {
          ...
          this._masterService.deleteData(apiUrl.salesEnquiry, `lc-details?where[LCDs_SysID]=${rowData.LCDs_SysID}&where[LCDSs_SysID]=${rowData.LCDSs_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              ...
              if (this._salesService.headerData.tblLCDetails.length === 0) {
                this.addRow('tblLCDetails', -1);
              }
            }
          }, err => {
            if (err.error.statusCode == 409) {
        ...
        if (table == 'tblLandedCost') {
          this._masterService.deleteData(apiUrl.salesEnquiry, `landed-cost?where[HTransLCost_SysID]=${rowData.HTransLCost_SysID}&where[HTransLCostS_SysID]=${rowData.HTransLCostS_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              this.isLoading = false
              this._messageService.add({ severity: 'success', summary: 'Row Deleted', detail: res.message });
              this._salesService.headerData.tblLandedCost.splice(index, 1);
              if (this._salesService.headerData.tblLandedCost.length === 0) {
                this.addRow('tblLandedCost', -1);
              }
            }
          }, err => {
            if (err.error.statusCode == 409) {
        ...
        if (table == 'tblLandedExp') {

          this._masterService.deleteData(apiUrl.salesEnquiry, `landed-expense?where[CSExp_SysID]=${rowData.CSExp_SysID}&where[CSExpS_SysID]=${rowData.CSExpS_SysID}`).then((res) => {
            if (res.success == false) {
              this.isLoading = false
              this._messageService.add({ severity: 'error', summary: 'Error', detail: res.message });
            } else {
              this.isLoading = false
              this._messageService.add({ severity: 'success', summary: 'Row Deleted', detail: res.message });
              this._salesService.headerData.tblLandedExp.splice(index, 1);
              if (this._salesService.headerData.tblLandedExp.length === 0) {
                this.addRow('tblLandedExp', -1);
              }
            }
          }, err => {
            if (err.error.statusCode == 409) {
  ...
  addRow(table: any, index: number) {
    if (table == 'tblTransDetail') {
      ...
      this._salesService.headerData.tblTransDetail.forEach(item => {
        item.Alld_Vou_Type = this._salesService.headerData.tblTransHead.Allh_Vou_Type
        item.Alld_VouType_Name = this._salesService.headerData.tblTransHead.Allh_VouType_Name
      });

    }
    if (table == 'tblExp') {
      const newRow = new TblExpDto()
      this._salesService.headerData.tblExp.splice(index + 1, 0, newRow);

    }
    if (table == 'tblTax') {
      const newRow = new TbltaxDto()
      this._salesService.headerData.tblTax.splice(index + 1, 0, newRow);

    }
    if (table == 'tblTaxAdd') {
      const newRow = new TbltaxAddDto()
      this._salesService.headerData.tblTaxAdd.splice(index + 1, 0, newRow);

    }
    if (table == 'tblTaxDetail') {
      const newRow = new tblTaxDetailDto()
      this._salesService.headerData.tblTaxDetail.splice(index + 1, 0, newRow);
    ...
    if (table == 'tblLandedCost') {
      const newRow = new HTransLCostDto()
      this._salesService.headerData.tblLandedCost.splice(index + 1, 0, newRow);

    }
    if (table == 'tblLandedExp') {
      const newRow = new TblLandedExpDto()
      this._salesService.headerData.tblLandedExp.splice(index + 1, 0, newRow);
