...
import { IaexpheadDto } from 'src/app/_dto/iaexphead.dto';
import { ExpenseGroupService } from 'src/app/_providers/expense-group.service';
import { ExpenseHeadService } from 'src/app/_providers/expense-head.service';
...
export class ExpenseHeadComponent extends ICTFormPageBean implements OnInit {

  DIALOG_TITLE = "Expense Head Master"
  API_END_URL = apiUrl.iaexphead;
  ...
  constructor(
    ...
    private _service: ExpenseHeadService,
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this._service.getLevel1().then((res) => {
      ...
      this.groupMasterData.push({
        label: 'Expense Head Master', data: { level: 0 },
        expanded: true,
  ...
  async preCreateInsert(): Promise<boolean> {
    if (!this.selectedFile) {
      // this._actionService.showToast(Toast.INFO, 'Expense Head Master', 'Please select any Level');
      return false;
    }
    else {
      if (this.selectedFile.data.level == 2) {
        await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("cannotCreateanySublevel") });
        return false;
      }
      if (this.selectedFile.data.level === 0)
        this._actionService.headerDto = IaexpgroupDto;
      if (this.selectedFile.data.level === 1)
        this._actionService.headerDto = IaexpheadDto;
    }
    return true;
  }
  ...
  async preSave() {
    if (!this.selectedFile) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("Pleaseselectanyrow") });
      return;
    }
    let level = this.selectedFile.data.level;
    if (level > 0 && !this.level1.code) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("grpCodeCannotbeBlank") });
      return;
    }
    if (level > 0 && !this.level1.name) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("grpNameCannotbeBlank") });
      return;
    }
    if (level > 0 && !this.level1.status) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("statusCannotbeBlank") });
      return;
    }
    if (level > 1 && !this.level2.code) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("subGrpCodeCannotbeBlank") });
      return;
    }
    if (level > 1 && !this.level2.name) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("subGrpNameCannotbeBlank") });
      return;
    }
    ...
    //   await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("categoryCannotbeBlank") });
    //   return;
    // }

    this.service.header = this.selectedFile.data;
    this.API_END_URL = level == 1 ? apiUrl.iaexpgroup : apiUrl.iaexphead;
    return true;
  }
  ...
  async postSave(): Promise<void> {
    ...
    this.API_END_URL = apiUrl.iaexphead;
  ...
  async preDelete(): Promise<boolean> {
    ...
    if (this.selectedFile.children.length > 0) {
      await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("childrenExist") });
      return;
    }
  ...
  async nodeUnselect(event) {
    ...
    await this._messageService.add({ severity: 'info', summary: 'Expense Head Master', detail: this._errorTanslateService.translate("userConfirmPasswordcannotbeBlank") });
  }
  ...
  getColumnInfo() {
    ...
    this.subGroupMetaData = [
      {
        type: 'group', size: 12, label: "", groupColumnMetaData: [
          { columnName: 'code', size: 4, labelCode: 'headCode', mandatory: true, maxlength: 10, disableExpression: () => this.isDisabled2 },
          { columnName: 'name', mandatory: true, size: 8, labelCode: 'headName', maxlength: 50 },
          { columnName: 'shortName', size: 4, labelCode: 'shortName', maxlength: 50 },
          { columnName: 'status', mandatory: true, size: 4, labelCode: 'status', type: 'dropdown', data: () => this.statusList },
