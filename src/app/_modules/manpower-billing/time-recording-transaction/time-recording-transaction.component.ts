...
@Component({
  selector: 'app-time-recording-transaction',
  templateUrl: './time-recording-transaction.component.html',
  styleUrls: ['./time-recording-transaction.component.scss']
})
export class TimeRecordingTransactionComponent implements OnInit {
 ...
 dayList: any[] = [];
  ...
  TblProjTransHead: TblProjTransHead = new TblProjTransHead()
    ...
    Allh_ProjMonth: null,
    TblProjTransDetail: [
      { selected: false, Alld_TcLDays: null, Alld_TcLDayWord: '', Alld_McalWoking: '', Alld_McalSunday: '', Alld_McalPubHoliday: '', Alld_TcLAttenSummary: '', Alld_Remark_SysID: null, Alld_Remark_Code: '', Alld_Remark_Name: '', Alld_Grid_SysID: null }
    ]
  };
  ...
  TableData = [
    { header: 'Time Card  Grid SysID', bind: 'Alld_SysID', width: '8rem', type: 'text', readOnly: true, hide: true, mandatory: true },
    ...
    { header: 'Designation Code', bind: 'Alld_Desig_Code', width: '12rem', type: 'text', PopUpData: 'Employee' },
    { header: 'Designation Name', bind: 'Alld_Desig_Name', width: '14rem', type: 'text' },


    { header: 'Select Site SysID', bind: 'Alld_Site_SysID', width: '8rem', type: 'text', readOnly: true, hide: true },
    { header: 'Select Site Code', bind: 'Alld_Site_Code', width: '12rem', type: 'text', PopUpData: 'Employee' },
    { header: 'Select Site Name', bind: 'Alld_Site_Name', width: '14rem', type: 'text' },


    { header: 'Select Job SysID', bind: 'Alld_Job_SysID', width: '8rem', type: 'text', readOnly: true, hide: true },
    { header: 'Select Job Code', bind: 'Alld_Job_Code', width: '12rem', type: 'text', PopUpData: 'Employee' },
    { header: 'Select Job Name', bind: 'Alld_Job_Name', width: '14rem', type: 'text' },


    { header: 'Attendance Days Per Camb(From Camp Records)', bind: 'Alld_Atten_Camp', width: '12rem', type: 'text', readOnly: true, hide: true },
    { header: 'Fixed Hours(From Master)', bind: 'Alld_FixedOT_Master', width: '12rem', type: 'text' },
    { header: 'Number Of Days Worked/Approx(System Calculation)', bind: 'Alld_Wokred_Days', width: '14rem', type: 'text' },


    { header: 'Total Working HRS As per Site without OT(System Calculation)', bind: 'Alld_ToLocation_SysID', width: '12rem', type: 'text', readOnly: true, hide: true },
    { header: 'Actual Working Hrs', bind: 'Alld_WorkHrs_Actual', width: '12rem', type: 'text' },
    { header: 'Load Attendance Calender', bind: 'Alld_SiteAtten_Entry', width: '14rem', type: 'text', PopUpData: 'LoadCalender' },



    { header: 'Actual Overtime', bind: 'Alld_Actual_OT', width: '8rem', type: 'text', hide: true },
    { header: 'HOT Working Hrs', bind: 'Alld_Actual_HOT', width: '10rem', type: 'text', },
 ...
 this.dayList = Array.from({ length: 31 }, (_, i) => ({
      label: (i + 1).toString(),
      value: i + 1
    }));
  ...
  Preview() {
   ...
   onMonthOrYearChange() {
    ...
    const year: number = Number(this.TblProjTransHead.Allh_ProjYear);
    const month: number = Number(this.TblProjTransHead.Allh_ProjMonth);

    if (!year || !month) return;

    const daysInMonth = new Date(year, month, 0).getDate();

    // ✅ Public holiday list (format: YYYY-MM-DD)
    ...
    this.TblProjTransHead.TblProjTransDetail = [];
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const isPublicHoliday = publicHolidays.includes(dateStr); // ✅ Declare this here

      this.TblProjTransHead.TblProjTransDetail.push({
        Alld_TcLDays: day,
        Alld_TcLDayWord: dayName,
        Alld_McalWoking: '',
        Alld_McalSunday: isSunday ? 'X' : '',
        Alld_McalPubHoliday: isPublicHoliday ? 'X' : '',  // ✅ No error now
        Alld_TcLAttenSummary: '',
        Alld_Remark_Code: '',
        Alld_Remark_Name: ''
      } as TblProjTransDetail);
    }
  }


  onDayTypeChange(rowData: TblProjTransDetail) {
    switch (rowData.Alld_McalDayType) {
      case 'W':
        rowData.Alld_TcLAttenSummary = 'P';
        break;
      case 'S':
        rowData.Alld_TcLAttenSummary = 'H';
        break;
      case 'H':
        rowData.Alld_TcLAttenSummary = 'HOT';
        break;
      default:
        rowData.Alld_TcLAttenSummary = '';
    }
}
}
