import { ActionService, FormService } from '@accurate/toolbar';
import { Component, OnInit, Input } from '@angular/core';
import { Iigrp7Dto } from 'src/app/_dto/iigrp7.dto';
import { IuserotherrightsDto } from 'src/app/_dto/iuserotherrights.dto';
import { PopUpLovService } from 'src/app/_providers/pop-up-lov.service';


@Component({
  selector: 'app-iuserotherrights-grp7',
  templateUrl: './iuserotherrights-grp7.component.html',
  styleUrls: ['./iuserotherrights-grp7.component.scss']
})
export class IuserotherrightsGrp7Component implements OnInit {

  @Input('parentData') public grp7Right;

  columns: { field: string; header: string; width: string, sort?: boolean, disabled?: boolean, filter?: boolean; disableAfterSave?: boolean; }[] = [];

  selectedDetailRow: IuserotherrightsDto;
  iigrp7LovData: Iigrp7Dto[] = [];
  iigrpColumns = [
    { field: 'code', header: 'Code', width: '5em' },
    { field: 'name', header: 'Name', width: '7em' },
  ];
  newRow: any;
  selectedRow: any;

  constructor(
    public service: FormService,
    private actionService: ActionService,
    public popUpService: PopUpLovService,
  ) { }

  ngOnInit(): void {
    this.initialize();
  }

  createInsertRow() {
    const newRow = new this.actionService.detailDto();
    this.actionService.detailPropertyName = "iuserotherrights";
    this.actionService.detailDto = IuserotherrightsDto;
    newRow.type = "GRP7";
    this.grp7Right.push(newRow);
  }

  deleteRow() {  
    let ri = -1;
    ri = this.grp7Right.indexOf(this.selectedRow);
    this.grp7Right.splice(ri, 1);
  }

  async onGrpCode(rowIndex: number) {
    const ref = this.actionService.showPopUp(this.iigrp7LovData, this.iigrpColumns);
    ref.onClose.subscribe((data: Iigrp7Dto) => {
      if (data) {
        this.grp7Right[rowIndex].code = data.code;
        this.grp7Right[rowIndex].name = data.name;
      }
    });
  }

  async initialize() {
    await this.popUpService.getGrp7Lov().then(data => {
      this.iigrp7LovData = data;
    });
  }

}
