...
export class CashBankSetupService {
  ...
  async getMasterData(apiEndUrl: string): Promise<any> {
   ...
   async getAllData(): Promise<any> {
    return await this.http.get<any>(`${environment.apiUrl}/${apiUrl.cashBankSetup}`).toPromise();
  }
  ...
  async saveSetup(body: any): Promise<IsetupDto> {
    return this.http.post<IsetupDto>(`${environment.apiUrl}/${apiUrl.setup}`,body).toPromise();
  }
