...
export class ReportConfigurationService {
  ...
  async getMasterData(apiEndUrl: string): Promise<IrepparamDto[]> {
    return await this.http.get<IrepparamDto[]>(`${environment.apiUrl}/${apiEndUrl}`).toPromise();
  }


  async updateConfigData(apiEndUrl: string,body:any): Promise<IrepparamDto[]> {
    return await this.http.put<IrepparamDto[]>(`${environment.apiUrl}/${apiEndUrl}`,body).toPromise();
  }

  async saveConfigData(apiEndUrl: string,data:any): Promise<IrepparamDto> {
    return await this.http.post<IrepparamDto>(`${environment.apiUrl}/${apiEndUrl}`,data).toPromise();
  }

  async deleteConfig(apiEndUrl:string):Promise<any>{
